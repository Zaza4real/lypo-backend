...
// ---- Admin: Update user (currently supports balance)
app.put("/api/admin/user-update", auth, asyncHandler(async (req, res) => {
  ...
  if (balanceRaw === undefined || balanceRaw === null || balanceRaw === "") {
    return res.status(400).json({ error: "MISSING_BALANCE" });
  }
  ...
  return res.json({
    ok: true,
    user: { email: updated.email, balance: Number(updated.balance || 0), created_at: updated.created_at, is_admin: isAdminEmail(updated.email) }
  });
}));

// ---- Admin: Delete user account (and related history)
app.delete("/api/admin/user-delete", auth, asyncHandler(async (req, res) => {
  ...
  // Delete related records first (safe if tables don't exist)
  try { await pool.query("DELETE FROM videos WHERE email=$1", [email]); } catch (e) {}
  try { await pool.query("DELETE FROM payments WHERE email=$1", [email]); } catch (e) {}
  try { await pool.query("DELETE FROM password_resets WHERE email=$1", [email]); } catch (e) {}
